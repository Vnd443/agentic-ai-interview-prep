...
**Why it matters:** This is how **search and RAG** work — turn your question and your documents into vectors, then find the closest ones. Foundational for topic [[04-embeddings-vector-search]]. Interview line: *"Embeddings turn meaning into geometry so 'similar' becomes 'close'."*
...
**Why it matters:** Ties straight to cost. Interview line: *"I match model size to task difficulty — small model for classification, big model only for hard reasoning."* (→ [[15-cost-optimization]])
...
**Why it matters:** Explains *"why are chat models decoder-only but embedding models encoder-based?"* → **generating** predicts the next word from the past; **understanding/search** benefits from seeing the whole sentence at once. (→ [[04-embeddings-vector-search]])
...
**Why it matters:** Knowledge comes from **pretraining** — so if the model doesn't *know* something, fine-tuning won't add facts; **RAG** is the fix. (→ [[24-fine-tuning]], [[05-rag]])
...
**Why it matters:** The classic design trade-off. Interview line: *"I'd use a small on-device SLM when latency, cost, or privacy matter, and a large cloud LLM only when the task needs the extra reasoning."* Ties to [[15-cost-optimization]] and edge deployment.
...
**Why it matters:** The cheapest way to steer a model — try examples **before** you ever consider fine-tuning. (→ [[03-prompt-engineering]])
...
**Why it matters:** Bigger window ≠ better answers. Stuffing it costs more and can *hurt* accuracy. (→ [[09-context-engineering]])
...
**Why it matters:** The #1 reason companies distrust LLMs in production. Power move: *"No single fix — RAG plus output validation, defence in depth."* (→ [[14-safety-guardrails]], [[06-evaluation]])
...
|---|---|---|---|
| **Prompt engineering** | The **wording** of one request | A single message | [[03-prompt-engineering]] |
| **Context engineering** | **Everything the model can see** — system prompt, RAG chunks, memory, history, tool outputs | One whole task/session | [[09-context-engineering]] |
| **Loop engineering** | **How the agent's think→act loop runs** — max iterations, stop condition, retries, budget guards | One agent's control flow | [[07-agents-tool-use]] |
| **Orchestration engineering** | **How many agents/steps** you wire together and how they hand off | A multi-agent system | [[12-agent-orchestration]] |
...
**Why it matters:** *"Every autonomous loop needs a max-iteration cap, a budget guard, and a clear termination condition — otherwise it's a runaway."* (→ [[07-agents-tool-use]], [[12-agent-orchestration]])
...
---
_Related: [[04-embeddings-vector-search]] · [[05-rag]] · [[03-prompt-engineering]] · [[09-context-engineering]] · [[12-agent-orchestration]] · [[15-cost-optimization]] · [[06-evaluation]]_
