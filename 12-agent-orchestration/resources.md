...
## Protocols
- **MCP (Model Context Protocol)** — agent ↔ tools/data → [[08-mcp-and-custom-skills]].
- **A2A (Agent-to-Agent)** — agent ↔ agent interop (Google-originated open spec). Search "A2A protocol spec".

## Tie-ins from this repo
- Single-agent tool use & ReAct → [[07-agents-tool-use]]
- Context isolation / sub-agent windows → [[09-context-engineering]]
- Evaluator loops → [[06-evaluation]]
- Cost of extra agents → [[15-cost-optimization]]
- Tracing which agent failed → [[16-monitoring-and-debugging]]
